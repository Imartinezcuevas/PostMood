...
def search_posts(request: SearchRequest):
    ...
    if cached_data:
        logging.info(f"[CACHE HIT] Returning cached results for '{keyword}'")
        return {
            "status": "done",
            "keyword": keyword,
    ...
    if existing_job_id:
        logging.info(f"[QUEUE] Job already enqueued for '{keyword}' ({existing_job_id})")
        return {
            "status": "queued",
    ...
    logging.info(f"Enqueued job {job.id} for keyword '{keyword}'")
